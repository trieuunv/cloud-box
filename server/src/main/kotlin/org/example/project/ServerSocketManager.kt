...
import org.example.project.dto.ClientSocket
import org.example.project.dto.SocketMessage
import java.util.concurrent.ConcurrentHashMap

object ServerSocketManager {
    private var serverSocket: ServerSocket? = null
    private val selectorManager = SelectorManager(Dispatchers.IO)
    private val clients = ConcurrentHashMap<String, ClientSocket>()
    ...
    suspend fun <T> sendToClient(client: ClientSocket, event: String, data: T, serializer: KSerializer<T>) {
        val jsonData = json.encodeToString(serializer, data)
        val socketMessage = SocketMessage(event, jsonData)
        val jsonSocket = json.encodeToString(SocketMessage.serializer(), socketMessage)
    ...
    private suspend fun <T> broadcast(event: String, data: T, serializer: KSerializer<T>) {
        val jsonData = json.encodeToString(serializer, data)
        val socketMessage = SocketMessage(event, jsonData)
        val jsonSocket = json.encodeToString(SocketMessage.serializer(), socketMessage)
    ...
    suspend fun <T> broadcastToRoom(room: String, event: String, data: T, serializer: KSerializer<T>) {
        val jsonData = json.encodeToString(serializer, data)
        val socketMessage = SocketMessage(event, jsonData)
        val jsonSocket = json.encodeToString(SocketMessage.serializer(), socketMessage)
